...
test.describe('SUAP IFBA', () => {
  test('axe-core: homepage', async ({ page, browser }, testInfo) => {
    const outputDir = `results/suap/${testInfo.project.name}`
    fs.mkdirSync(outputDir, { recursive: true })

    await page.goto('https://suap.ifba.edu.br')
  ...
  test('IBM Equal Access: homepage', async ({ page, browser }, testInfo) => {
    ...
    const results = await aChecker.getCompliance(page, `suap-homepage-${testInfo.project.name}`)
